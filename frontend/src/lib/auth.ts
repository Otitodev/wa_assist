  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      await resend.emails.send({
        from: "HybridFlow <otito@addpost.site>",
        to: user.email,
        subject: "Reset your HybridFlow password",
        html: `
          <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px;">
            <h2 style="color:#16a34a;">Reset your password</h2>
            <p>Hi ${user.name || user.email},</p>
            <p>Click the button below to reset your HybridFlow password. This link expires in 1 hour.</p>
